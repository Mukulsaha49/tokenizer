from hwr.tokenizer import tokenize
import json
...
class HRDataset(Dataset):
    ...
    def __init__(
        ...
        vocab_path = os.path.join(self.dir_ds, "/home/mukul36/Documents/REWI/pd_wi_hw5_word/token_vocab.json")
    ...
    def __getitem__(self, idx: int) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        # Tokenize the label (e.g., "hello" → ["he", "el", "ll", "lo"])
        tokens = tokenize(label, n=2)

        # Convert tokens to indices using the vocab
        label_indices = [self.token_to_index.get(token, self.token_to_index["<UNK>"]) for token in tokens]
